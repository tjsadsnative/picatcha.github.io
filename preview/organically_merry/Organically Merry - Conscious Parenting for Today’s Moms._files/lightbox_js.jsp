...
/* FILE: /js/jquery/jquery-1.10.2.modified.js */
/*!
...
jQuery.extend({
	...
	attr: function( elem, name, value ) {
		/* INFUSIONSOFT: ADDED NULL CHECK */
        if (!elem) {
            return;
        }
        /* INFUSIONSOFT: THE END :( */
...
if ( typeof module === "object" && module && typeof module.exports === "object" ) {
    ...
    /* INFUSIONSOFT: ADDED FROM JQUERY 1.6.2 FOR SUPPORT OF EXISTING CODE */
...
// Chrome is Webkit, but Webkit is also Safari.
    ...
    /*
     * curCSS issue: .curCSS has been removed in jQuery 1.8; the replacement is .css; however, since some of our
     * code uses older versions of jQuery-UI which in turn depends on .curCSS, we carry it forward here.
     * This should solve javascript errors like "TypeError: c.curCSS is not a function".
     */
    jQuery.curCSS = jQuery.css;

    /*
     * propAttr issue: Some of our code uses older versions of jQuery-UI which contain a definition of propAttr like this:
     *          propAttr: $.fn.prop || $.fn.attr;
     * ...which is not defined in later versions of jQuery-UI.  We define it here to bridge the gap.
     */
    jQuery.fn.extend({
        propAttr: jQuery.fn.prop || jQuery.fn.attr
    });

    /* INFUSIONSOFT: END OF ADDITION */
...
window.BUILD_NUMBER = "1.43.0.46"; // TODO: Add actual build number here
...
(function () {
        ...
        if (module && module.dependencies) {
            ...
            if (jsD && jsD.length) {
                ...
                for (var i=0; i<jsD.length; i++) {
                    ...
                    if (jsDependency == checkedModules[jsDependency] || (modulesWaiting[jsDependency] && hasCircularDependency(jsDependency, jQuery.extend({},checkedModules)))) {
    ...
    function checkModuleDependencies() {
        var modulesNoLongerWaiting = [];
        var numModulesWaiting = 0;
        var i, path;
        var recheck = false;
        var module;
        for (path in modulesWaiting) {
            numModulesWaiting++;
            ...
            if (allDependenciesMet) {
                modulesNoLongerWaiting.push(path);
            }
        }
        // Remove modules from waiting list first before executing as they might trigger another check
        for (i=0; i<modulesNoLongerWaiting.length; i++) {
            path = modulesNoLongerWaiting[i];
            delete modulesWaiting[path];
        }
        // Execute modules that are no longer waiting
        for (i=0; i<modulesNoLongerWaiting.length; i++) {
            path = modulesNoLongerWaiting[i];
            module = modules[path];
            if (module) {
                ...
                log.warn('Module path no longer exists: %o', path);
            }
        }
        if (recheck && (numModulesWaiting - modulesNoLongerWaiting.length > 0)) {
            checkModuleDependencies();
        }
    }
    ...
    ;Infusion.scriptLoaded('/js/jquery/jquery-1.10.2.modified.js');
...
Infusion.Utils.escapeId = function(id) {
...
Infusion.Utils.useDefaultIfNotDefined = function (what, def) {
    return Infusion.Utils.isDefined(what) ? what : def;
};

Infusion.Utils.isDefined = function (what) {
...
Infusion("Ajax", function () {
    ...
    // Handle page unload vs. app being down
    // src: http://stackoverflow.com/questions/866771/jquery-ambiguous-ajax-error
    var navigatingAway = false;
    jQuery(window).bind('beforeunload', function() { navigatingAway = true; });
    ...
    function getHtml(options) {
        ...
        return executeAjax("html", options);
    }

    function getJson(options) {
        return executeAjax("json", options);
    }

    function executeAjax(dataType, options) {
    ...
    function validateOptions(options) {
        ...
        validated.requestType = useDefaultIfNotDefined(options.requestType, "POST");
        ...
        validated.complete = useDefaultIfNotDefined(options.complete, function () {});
    ...
    function advancedErrorHandler(xhr, status, error, options) {
        // user is navigating away not an error state
        if ((status == null || xhr.status == 0) && navigatingAway) {
            return;

            // the server went down
        } else if ((status == null || xhr.status == 0) && !navigatingAway) {
    ...
    function ajaxFill(options) {
        ...
        var shouldShowSpinnerOnCenter = useDefaultIfNotDefined(options.showSpinnerOnCenter, false);
        var processData = useDefaultIfNotDefined(options.processData, true);
        var contentType = useDefaultIfNotDefined(options.contentType, "application/x-www-form-urlencoded; charset=UTF-8");
        ...
        var originalErrorHandler = options.errorHandler;
        var errorHandler = function (XMLHttpRequest, textStatus, errorThrown) {
            if (originalErrorHandler)
                originalErrorHandler(XMLHttpRequest, textStatus, errorThrown, $target);
            else
        ...
        jQuery.ajax({
            ...
            complete:afterFill,
            processData: processData,  // whether or not to process the data (true/false)
            contentType: contentType   // what the contentType is
        });
    }
    ...
    function showSpinner(options) {
        ...
        // Start adding the spinner HTML
        spinnerHtml += " ><tr><td><img src=\"/slices/ajax-spinner.gif\"";

        if (options.spinnerStyle) { // Give the spinner image the supplied class
            spinnerHtml += " class=\"" + options.spinnerStyle + "\"";
        }

        // Continue building the spinner HTML
        spinnerHtml += "></td><td";

        if (options.spinnerTextStyle) { // Give the spinner text the supplied class
            spinnerHtml += " class=\"" + options.spinnerTextStyle + "\"";
        }

        // End the spinner HTML
        spinnerHtml += ">" + spinnerText + "</td></tr>";
...
Infusion("UI", function() {
    ...
    ns.hide = hide;
    ns.focus = focus;
    ns.focusFirstTextInput = focusFirstTextInput;
    ...
    ns.applyTooltipIfEllipsed = applyTooltipIfEllipsed;
    ...
    function focus(selector) {
        //Internet Explorer does not handle jQuery.focus() properly. It needs additional time to reference the dom, therefore a small timeout is required
        setTimeout(function() {
    ...
    function focusFirstTextInput() {
        var $firstInput = jQuery(".ui-tabs-panel").find('input[type=text],textarea').filter(':visible:first');
        if ($firstInput != null) {
            $firstInput.focus();
        }
    }
    ...
    /**
     * It makes the <code>container</code> show the <code>tooltip</code> text only when it is being truncated by ellipsis.
     * @param container
     * @param tooltip
     */
    function applyTooltipIfEllipsed(container, tooltip){
        if( isEllipsed(container) ){
    ...
    /**
     * Returns true only when <code>obj</code> has the ellipsis text overflow and its content is wide enough to have the ellipsis shown.
     * @param obj Object upon the test will run against.
     * @return {Boolean}
     */
    function isEllipsed(obj) {

        if (jQuery(obj).css('text-overflow') == ("ellipsis")) {
            //clones the object to be tested but with an unlimited overflow so we know what the real width of the object would be if not ellipsed.
            var cloneUnTruncated = jQuery(obj).clone()
                                        .css( {     overflow:'auto',
                                                    visibility:'hidden',
                                                    'max-width': 'none'} )
                                            .appendTo('body');
            var untruncatedWidth = jQuery(cloneUnTruncated).width();
            var truncatedWidth = parseInt(jQuery(obj).css("max-width"));
            cloneUnTruncated.remove();
            //if the untruncated width is longer than the original, means the original had ellipsis applied to it.
            return untruncatedWidth > truncatedWidth;
...
Infusion("Url", function() {
    ...
    ns.getWizardUrl = getWizardUrl;
    ns.param = param;
    ns.appendParam = appendParam;
    ns.appendParams = appendParams;
    ns.checkForAndCreateHttpIfNoneExists = checkForAndCreateHttpIfNoneExists;
    ns.parseQueryString = parseQueryString;
    ns.getUrlParts = getUrlParts;
    ns.getCampProxyViewUrl = getCampProxyViewUrl;
    ns.getCampProxyActionUrl = getCampProxyActionUrl;
    ns.getCampProxyJsonUrl = getCampProxyJsonUrl;
    ns.urlEncode = urlEncode;

    var PREFIX_APP = "/app/";
    var PREFIX_WIZARDS = "wizards/";
    ...
    function getJsonUrl(controller, action) {
        return getUrl(controller, action, EXTENSION_JSON, null);
    }

    function getWizardUrl(wizardId) {
        return getUrl(PREFIX_WIZARDS + wizardId, null, null, null);
    }

    function getUrl(controller, action, extension, id) {
        var url = PREFIX_APP + controller;
    ...
    function appendParams(url, params) {
        var param;
        for (param in params) {
            url = appendParam(url, param, params[param]);
        }
        return url;
    }
    ...
    function parseQueryString(url) {
        ...
        jQuery.each(pairs, function(indexInArray, valueOfElement) {
            var pair = valueOfElement.split('=');
    ...
    function getCampProxyViewUrl(controller, action, queryString) {
        var url = getViewUrl("campProxy", "view");

        return appendCampProxyParams(url, controller, action, queryString);
    }

    function getCampProxyActionUrl(controller, action) {
        var url = getViewUrl("campProxy", "action");

        return appendCampProxyParams(url, controller, action);
    }

    function getCampProxyJsonUrl(controller, action) {
        var url = getJsonUrl("campProxy", "jsonPost");

        return appendCampProxyParams(url, controller, action);
    }

    function appendCampProxyParams(url, controller, action, queryString) {
        url = appendParam(url, "controller", controller);
        url = appendParam(url, "action", action);

        if (queryString) {
            url = appendParam(url, "queryString", urlEncode(queryString));
        }
...
Infusion("jQuery", function() {
    ...
    ns.selectFirstRadioOption = selectFirstRadioOption;
    ns.isChecked = isChecked;
    ns.setChecked = setChecked;
    ns.escapeHtml = escapeHtml;
    ns.unescapeHtml = unescapeHtml;
    ns.isDraggable = isDraggable;
    ns.isAccordion = isAccordion;
    ns.isIE = isIE;
    ...
    function selectFirstRadioOption(name) {
        var firstOption = jQuery("input[name='" + name + "'][disabled=false]:first");
        firstOption.prop('checked', true);

        return firstOption.val();
    }
    ...
    function setChecked(id, checked) {
        return jQuery("#" + id).prop("checked", checked);
    }
    ...
    function isDraggable($el) {
        return $el.is(":ui-draggable");
    }

    function isAccordion($el) {
        return $el.is(":ui-accordion");
    }

    function isIE() {
        // Regex pulled from http://stackoverflow.com/questions/20089145/how-to-detect-ie11-using-jquery
        var isIE11 = !!navigator.userAgent.match(/Trident.*rv\:11\./);
        return jQuery.browser.msie || isIE11;
    }
...
Infusion("Popup", function() {
    ...
    function openLightbox(fillDivId, url, config, contentId, data) {
        ...
        if (url) {
            if (config.useiframe) {
                $container.append("<iframe id='iframe_" + fillDivId + "'name='iframe_" + fillDivId + "' scrolling='no' frameborder='0' height='" + (config.height) + "px' width='" + (config.width) + "px' src='" + url + "'></iframe>");
            } else {
                jQuery.ajax({
                    type: "GET",
                    async: false,
        ...
        jQuery("#theLightboxContainer").removeClass('off-page-load');
        ...
        if (config.startcentered && autoWidth) {
            ...
            calculateLocation(config, $content, $win, $doc);
            jQuery('#' + contentId + "_c").css("left", config.x).css("top", config.y);
        ...
        // it is getting a shadow class that is overriding the default shadow
        jQuery('#' + contentId + "_c").removeClass('shadow');
    ...
    function getLightboxHTML(title, msg, config) {
        ...
        if (title != null && typeof title !== "undefined" && title.replace(/ /g, "").length > 0) {
            html += '<div class="hd">' + Infusion.jQuery.escapeHtml(title) + '</div>';
        }
    ...
    function fetchOrCreateLightboxContainer() {
        ...
        if (container.length == 0) {
            container = jQuery("<div id='theLightboxContainer'></div>").appendTo(jQuery("body:first"));
        }
        container.addClass('off-page-load');
...
function centerWindow() {
    ...
    if (window.opener) {
        ...
        if (left < 645) {
            ...
            window.moveTo(left, top - 50);
            //There is a chrome bug, sometimes when moveTo is called it re-sizes the popup window to a very small size.
            //Calling resizeTo below with the current height and width to resolve the problem.
...
function openAdminHW(url, height, width, windowName) {
    ...
    var winProp = 'width=' + width + ',height=' + height + ',left=' + winleft + ',top=' + winUp + ',scrollbars=yes,resizable=yes';
...
function clearElement(elementName, label) {
    jQuery(document.getElementById(elementName)).val(0);
    var textToDisplay = 'Choose ' + Infusion.Utils.getIndefiniteArticle(label);
    jQuery(document.getElementById(elementName + "_Name")).val(textToDisplay);
}
