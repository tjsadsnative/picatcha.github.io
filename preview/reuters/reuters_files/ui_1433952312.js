...
function force_clickmode(){VM._.each($iTXT.ui.HookManager.hooks,function(hook){hook.ad.params.set("cts",0,99999);if(hook.ad.params.getBool('hk.autopeek.on',true)){hook.ad.params.set("hk.click.mode",0,$iTXT.cnst.WEIGHTING_DEFAULT_CAMPAIGN);}else{if(hook.ad.params.get('hk.click.mode')==0||!hook.ad.params.get('hk.click.mode')){hook.ad.params.set("hk.click.mode",1,$iTXT.cnst.WEIGHTING_DEFAULT_CAMPAIGN);}}});}
...
function autoplay(){if(!$iTXT.ui.tt.currentAdvert||$iTXT.ui.tt.hasClosed){if(all_views<page_lim){var candidates=[];var check_func;VM._.each($iTXT.ui.HookManager.hooks,function(hook){if(hook.isVibrantView()){check_func=views_ok;if(hook.ad.params.getBool("js.autoplay.byiono",false)){check_func=iono_ok;}
...
return(this.ad.params.get('js.autoplay',false)||VM._.some(adtypes,function(adt){return adt===this.ad.$A.at;},this))&&$iTXT.glob.params.getBool('js.autoplay.channel',false);},hasMobt:function(){return $iTXT.ui.MobtManager.isMobtEnabled(this);},applyResetStyle:function(){this.rootElement.style.cssText=['font-weight: normal','font-size: 100%','font-style: normal','text-decoration: none','border: 0px none transparent','padding: 0px','background-color: transparent','background-image: none','display: inline'].join(';');},setAdvert:function(ad)
...
if(!$iTXT.ui.tt.isOpen||$iTXT.ui.tt.currentAdvert.hook!==this){setTimeout(this._launcher,60);}
...
return;this.options.ps=opts.state;var hookId=$iTXT.ui.tt.currentHookId;var hook=document.getElementById(hookId);var pos=$iTXT.core.Util.getCurrentPosition(hook);opts.left=pos.left;$iTXT.ui.tt.open(opts.left,opts.top,opts.state);},_resizeTooltip:function()
...
$iTXT.ui.TooltipPosition={AR:"AR",AC:"AC",AL:"AL",CL:"CL",CR:"CR",BR:"BR",BC:"BC",BL:"BL"};$iTXT.ui.TooltipPlacer={STEP_LENGTH_PX:1,TAIL_WIDTH:30,weight:{AC:90,AL:85,AR:80,BC:70,BL:65,BR:60},_getHookPosition:function(hook){if(hook.hasMobt()&&hook.mobtCover){return $iTXT.core.Util.getPosition(hook.mobtCover);}
return hook.getPosition();},_getHookSize:function(hook){var el=hook.rootElement;if(hook.hasMobt()&&hook.mobtCover){el=hook.mobtCover;}
