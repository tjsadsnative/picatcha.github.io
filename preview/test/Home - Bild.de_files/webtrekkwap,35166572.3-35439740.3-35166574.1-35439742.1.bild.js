...
/*webtrekk-videoWap:35439740.3*/

function videoTrackingEvent(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, ivw2VideoData) {
    wt_init_media("pixel.bild.de", "664854012571618");
    ...
    try {
        ...
        if (u) {
            ...
            var w = "";
            if (u === "init") {
                w = "ck1=" + i + ";ck2=" + j + ";ck3=" + v + ";ck4=" + k + ";ck8=" + i + "/" + m + ";ck9=" + l + ";ck13=" + q + ";mg1=" + k;
                if (r) {
                    w += ";ck14=" + r;
                }
            }
            switch (b) {
                case "PlayAutoRev":
                    if (u === "init") {
                        w = w !== "" ? w + ";ck12=autoplay" : "ck12=autoplay";
                    }
                    break;
                case "JUMPLABEL":
                    w = w !== "" ? w + ";ck15=" + s : "ck15=" + s;
            ...
            if (u == "eof") {
                wt_sendinfo_media(g, "end", c, h, w, "", "", "");
                wt_sendinfo_media(g, "eof", c, h, w, "", "", "");
            } else {
                wt_sendinfo_media(g, u, c, h, w, "", "", "");
            }
        }
    } catch (x) {}

    if ((b === "PLAY" || b === "playClick")  && (typeof o !== "undefined" && o !== null)) {
        try {
            window.iom.c(createVideoCountObject(ivw2VideoData), 2);
        } catch (err) {
            console.warn("videoTrackingEvent:: Failed to call IVW2 CountVideo Method.");
        }
    }

    function createVideoCountObject(ivw2Data){

        return {
            st: SETTINGS.ivw2Defaults.st,
            cp: ivw2Data.ivw2Code,
            sv: SETTINGS.ivw2Defaults.sv,
            co: SETTINGS.ivw2Defaults.co,
            contentCode: ivw2Data.contentCode
        };

    }

}
/*wtsendtimeparams:35166574.1*/
...
function wt3_send_timeparams(params, dest) {
    ...
    if (dest == 1) {
        wt.registerEvent(window, wt.isIE && wt.wtTypeof(window.onbeforeunload) ? "beforeunload" : "unload", function() {
...
function wt_init_media(trackDomain, trackId, sampling) {
    webtrekkMediaTracking.mediaStVersion = 316;
...
function wt_sendinfo_media(wt_mi, mk, mt1, mt2, mg, bw, vol, mute) {
    ...
    if (typeof mt2 != "undefined" && mt2 != "" && mt2 && (typeof webtrekkMediaTracking.posInterval[wt_mi] == "undefined" || webtrekkMediaTracking.posInterval[wt_mi] == "")) {
        webtrekkMediaTracking.posInterval[wt_mi] = parseInt(mt2) / wt_re >= 10 ? parseInt(mt2) / wt_re * 1e3 : 10 * 1e3;
        ...
        webtrekkMediaTracking.posInterval[wt_mi] = 3 * 2 * 5 * 2 * 1e3;
    }
