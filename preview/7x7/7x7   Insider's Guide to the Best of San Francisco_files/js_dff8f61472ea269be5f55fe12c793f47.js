...
;
Drupal.behaviors.sevenbyseven_carousel = function (context) {
        $('#promotions').show();
	//invoke ye olde jcarousel
	$('#promotions-carousel').jcarousel({
	wrap: 'both',
	scroll: 1
	});
	
	$('.promotions-list li').removeClass('last');
	$('#promotions div').removeClass('item-list');
...
$(document).ready(function(){
	...
	function refreshAds(){
	  if($('#ad-728x90-top').length){
	    $('#ad-728x90-top').attr("src", $('#ad-728x90-top').attr("src"));
	  }
	  if($('#ad-728x90-bottom').length){
	    $('#ad-728x90-bottom').attr("src", $('#ad-728x90-bottom').attr("src"));
	  }
	  if($('#ad-300x250-sidebar').length){
	    $('#ad-300x250-sidebar').attr("src", $('#ad-300x250-sidebar').attr("src"));
	  }
	}

	//$("ul.subnav").parent().append("<span></span>"); //Only shows drop down trigger when js is enabled (Adds empty span tag after ul.subnav*)

	/* $("#subnav-header").click(function() { //When trigger is clicked...
		$(this).addClass("subhover");
		//Following events are applied to the subnav itself (moving subnav up and down)
		$("#subnav").slideDown('medium').show(); //Drop down the subnav on click

		$(this).parent().hover(function() {
		}, function(){
			$(this).parent().find("#subnav").slideUp('slow')
			$('#subnav-header').removeClass("subhover"); //When the mouse hovers out of the subnav, move it back up
		});
...
*/
	...
	$('#galleria .galleria-image-nav-left, #galleria .galleria-image-nav-right, .galleria-thumbnails .galleria-image').click(function(){
		refreshAds();
