...
/*!
 *  Echo - Lazy Loading 2013, Todd Motto
...
window.Echo = (function (window, document, undefined) {
  ...
  var renderImage = function(image, index) {
    requestAnimationFrame(function(){
      image.src = image.getAttribute('data-echo');
      image.removeAttribute('data-echo');
      store.splice(index, 1);
      if(image.classList.contains('lazy-load')) {
  ...
  var _pollImages = function(renderAll) {
  ...
  var _throttle = function(renderAll) {
    clearTimeout(poll);
    poll = setTimeout(function() {
      _pollImages(renderAll)
    }, throttle);
  };

  var init = function (obj, renderAll) {
...
(function ( $, window, document, undefined ) {
  ...
  popup = {
    googlePlus: function(opt){
      window.open("https://plus.google.com/share?hl="+opt.buttons.googlePlus.lang+"&url="+encodeURIComponent((opt.buttons.googlePlus.url !== '' ? opt.buttons.googlePlus.url : opt.url)), "", "toolbar=0, status=0, width=900, height=500, left="+((screen.width/2)-( 900 / 2))+", top="+((screen.height/2)-( 500 / 2)) );
    },
    facebook: function(opt){
      window.open("http://www.facebook.com/sharer/sharer.php?u="+encodeURIComponent((opt.buttons.facebook.url !== '' ? opt.buttons.facebook.url : opt.url))+"&t="+opt.text+"", "", "toolbar=0, status=0, width=900, height=500, left="+((screen.width/2)-( 900 / 2))+", top="+((screen.height/2)-( 500 / 2)) );
    },
    twitter: function(opt){
      window.open("https://twitter.com/intent/tweet?text="+encodeURIComponent(opt.text)+"&url="+encodeURIComponent((opt.buttons.twitter.url !== '' ? opt.buttons.twitter.url : opt.url))+(opt.buttons.twitter.via !== '' ? '&via='+opt.buttons.twitter.via : ''), "", "toolbar=0, status=0, width=650, height=360, left="+((screen.width/2)-( 650 / 2))+", top="+((screen.height/2)-( 360 / 2)) );
    },
    digg: function(opt){
    ...
    pinterest: function(opt){
      window.open('http://pinterest.com/pin/create/button/?url='+encodeURIComponent((opt.buttons.pinterest.url !== '' ? opt.buttons.pinterest.url : opt.url))+'&media='+encodeURIComponent(opt.buttons.pinterest.media)+'&description='+opt.buttons.pinterest.description, 'pinterest', 'toolbar=no,width=700,height=300, left='+((screen.width/2)-( 700 / 2))+', top='+((screen.height/2)-( 300 / 2)));
    }
  };
