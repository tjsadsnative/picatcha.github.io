...
$("#inline").appendTo(".entry-content>p:nth-child(2)");
	}
...
if ($("body").hasClass("single-post")) {
		...
		var load_posts = function(){ 
			 $.ajax({
				...
				data: {
					...
					pageNumber: page,
					query : ajaxpagination.query,
					trending : ajaxpagination.page,
					},
  ...
  (function($) {
	...
	$(document).on( 'click', '.fancy-purple-week', function( event ) {
		event.preventDefault();
		var load_posts = function(){ 
			 $.ajax({
				url: ajaxpagination.ajaxurl,
				...
				success: function( result ) {
					...
					$(".fancy-purple-week h3").text('No more stories to load');
					}else{
					$(".more").append(result);
					page++;
					}
...
if ($("body").hasClass("single-post")) {
    ...
    footTop = $('.article-nav').offset().top - parseFloat($('.article-nav').css('marginTop').replace(/auto/, 0));
    maxY = footTop - 360;
