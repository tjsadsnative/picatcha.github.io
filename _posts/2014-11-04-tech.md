...
## 1. Open RTB v2.3 Request

### 1.1 What to expect in bid request
1. AdsNative exchange usually requires following creative assets from DSPs,
  * URL, Title, Image and Brand Name, and that should be expected as required field in native object.
  * Summary is highly recommended but usually not required
  * 'ctatext' is usually only shown for app install ads in ANX
  * The main image asset for the native ad is requested as type 3 from Native 1.0 specs. 
  * ANX bid requests usually contains wmin and hmin which should be strictly considered as minimun image size required in the response. 
  * bid request will also contains 'w' and 'h' value which is good representation of recommended image ratio. DSPs are NOT expected to resize the image to fit to mentioned image size. ANX will resize or center the image to fit the placement.    
2. For 'Recommendation Widgets', 'plcmtcnt' will be greater than 1 other it's usually defaulted to 1. In case of 'plcmtcnt' > 1, DSPs can bid with multiple 'bid' objects. 
3. ANX requires DSPs to pass 'nurl' (seatbid.bid.nurl) for win notice. ANX supports all macros defined in Table 4.4 in OpenRTB 2.3 specs. Win notice is sent when an ad impression is served/rendered on the client side.
4. All the bid pricing in request and response is only processed in USD.
5. If DSP has cookie sync URL integrated with ANX, in bid request they can expect 'buyeruid' in 'user' object.
...
![](https://s3.amazonaws.com/adsnative-public/images/infeed.png)

#### End of Article (502) Example

![](https://s3.amazonaws.com/adsnative-public/images/end-of-article.png)

#### Recommendation Widgets (2) Example

![](https://s3.amazonaws.com/adsnative-public/images/recommendation-widget.png)

## 2. OpenRTB v2.3 Example Bid Request
...
## 3. OpenRTB v2.3 Example Bid Response
