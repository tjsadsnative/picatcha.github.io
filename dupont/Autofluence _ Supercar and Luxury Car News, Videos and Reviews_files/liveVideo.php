...
function SekindoAdsManager(uniqueID, config, containerDiv)
{
    ...
    this.waterFallLifeTime = 0;
    this.waterFallLifeTimeOut = null
...
SekindoAdsManager.prototype.loadAdsProgram = function loadAdsProgram()
{
    ...
    function transferComplete(evt)
    {
        ...
        try
        {
            ...
            if(json.waterFallLifeTime != undefined && (json.waterFallLifeTime > 0))
            {
                ref.waterFallLifeTime = parseFloat(json.waterFallLifeTime);
                if(ref.waterFallLifeTimeOut)clearTimeout(ref.waterFallLifeTimeOut);
                if(ref.waterFallLifeTime > 0)
                {
                    ref.waterFallLifeTimeOut = setTimeout(function(){ref.loadAdsProgram();},ref.waterFallLifeTime);
                }
            }
...
{
    var ref = this;
    if(this.newAdsProgram)
    {
        this.adsProgram = this.newAdsProgram;
        this.newAdsProgram = null;
        this.adsProgramIndex = -1;
    }
...
SekindoAdsManager.prototype.adsLoaded = function adsLoaded(ads)
{
    ...
    if(this.isFlashVpaid && !this.config.supportFlashPlayerVersion)//we got flash vpaid but browser don't support it.
    {
        this.queryVasts();
        return;
    }
...
{
    var ref = this;
    if(this.adsProccessPaused)return;
    this.adType = 'linear';
...
SekindoAdsManager.prototype.generateVideoElement = function generateVideoElement()
{
    ...
    this.videoElement.id = 'adVideoElement';
...
SekindoAdsManager.prototype.onVpaidEvent = function onVpaidEvent(val)
{
    ...
    switch(type)
    {
        ...
        case 'onAdImpression':
            ...
            if(this.videoElement && this.videoElement.parentNode)this.videoElement.parentNode.style.display = 'block';

            this.vpaidSlot.style.zIndex = SekindoUtils.getHighestZIndex(this.config.iframewindow, 'div')+1;
            this.vpaidSlot.style.left = "0px";
        ...
        case 'onAdVideoComplete':
            ...
            this.currAd.track('complete');
            this.videoCompleteTimeout = setTimeout(function(){ref.onAdComplete();},1000);
            break;
        ...
        case 'onStartAd':
            ...
            break;
        case 'onStopAd':
            if(this.videoCompleteTimeout)clearTimeout(ref.videoCompleteTimeout);
            this.onAdComplete();
            break;
        case 'onSkipAd':
            this.onAdComplete();
...
function SekindoVideoInterface(videoDiv)
{
    ...
    this.videoDiv.__defineGetter__("width", function(){
        return ref.jpgSeqObj.config.width;
    });
...
function SekindoCBar(uniqueID, config)
{
    ...
    this.jenerateButton = function(name, callBackFunc, imgUrl, width, height  )
    {
        ...
        btn.id = name;
        btn.src = this.config.absolutePath+imgUrl;
    ...
    var relativePath = '/content/video/splayer/assets/';
    this.playBtn = this.jenerateButton("Play", this.onButton, relativePath+'playBtn.png');
    this.pauseBtn = this.jenerateButton("Pause", this.onButton, relativePath+'pauseBtn.png');
    this.muteBtn = this.jenerateButton("Mute", this.onButton, relativePath+'muteBtn.png');
    this.unMuteBtn = this.jenerateButton("UnMute", this.onButton, relativePath+'unMuteBtn.png');
    if(this.config.showLogo == 'true')
    {
        this.logoBtn = this.jenerateButton("Logo", this.onButton, this.config.logoPath);
...
SekindoVPAIDWrapper = function(config, VPAIDUrl, creativeData, environmentVars, uniqueID)
{
    ...
    function onScriptLoad()
    {
        ...
        if (!ref.checkVPAIDInterface(ref._creative))
        {
            ...
            this.onAdError('vpaidInterfaceNotConformed');
            return;
        }
...
{
    var ref = this;
    if(this.killTimeOut)clearTimeout(ref.killTimeOut);
    if(this.impressionTimer)clearTimeout(ref.impressionTimer);
...
SekindoUtils.getHighestZIndex = function  (win, selector) {
    // No granularity by default; look at everything
    if (!selector) { selector = '*' };
    var w = win || window;
    var elements = w.document.querySelectorAll(selector),
        i = 0,
        e, s,
        max = elements.length,
        found = [];
    for (; i < max; i += 1) {
        e = w.getComputedStyle(elements[i], null).zIndex || elements[i].currentStyle.zIndex;
        s = w.getComputedStyle(elements[i], null).position || elements[i].currentStyle.position;
...
SekindoView.prototype.construct = function construct()
{
    ...
    this.containerDiv.id = 'viewContainer';
    ...
    this.videoDiv.id = 'videoContainer';
...
SekindoSPlayer.prototype.constructContainer = function constructContainer(iframe)
{
    ...
    this.containerDiv.id = 'mainContainer';
...
SekindoSPlayer.prototype.onVideoEvent = function onVideoEvent(_videoElement, val)
{
    ...
    {
        case 'volumechange':
            var muted = (this.videoElement)?this.videoElement.muted:true;
            ...
            setTimeout(function()
            {
                window['sekindoBus'+ref.uniqueID].triggerNote('onVideoEvent', {type:'volumechange',val:muted});
            }, 100);
            break;
        case 'playing':
...
// <script type="text/javascript">
	...
	configPlayer.adsTimeGap = 3;//in seconds - Number
	configPlayer.waterFall = 'http://live.sekindo.com/live/liveView.php?s=57993&vid_debugInfo=6386444_apn_0_0_3258_0_blog.dupontregistry.com&vid_vastTimeout=-1&vid_content_url=https%3A%2F%2Fvideo.sekindo.com%2Fuploads%2Fvideo%2Fseasonal_depression_mini.mp4&vid_content_id=seasonal_depression&vid_content_desc=Life+Therapy+Series+-+Seasonal+Depression&vid_content_title=Health+-+Seasonal+Depression&vid_content_duration=80&vid_vastType=3&vid_waterfall=auto&vid_playerVer=&ri=&pubUrl=http%3A%2F%2Fblog.dupontregistry.com%2F%3Fadsnative_preview%3D1%26cid%3D709&cbuster=${CBUSTER}' + '&vid_vpaidFlash=' + (supportFlashPlayerVersion ? '1' : '0');
	configPlayer.playlistMultiplier = 25; // (0 ...Infinity) - int
	configPlayer.contentClickUrl = '';
	configPlayer.contentPlayList = [{
				"url": "https://video.sekindo.com/uploads/video/seasonal_depression_mini.mp4",
				"title": "Health - Seasonal Depression"
			}];
	...
	configPlayer.isNative = isNative;
