...
;(function () {
    function sdOffsetScroll(id)
    {
        if ($(id).length > 0)
        {
            setTimeout(function () {
                var base = location.hash.match(/post\d+/) ? 32 : 8;
                var offset = (base + $("#globalBar:not([data-nosticky])").outerHeight() + $("#dealStats:not(.expiredDeal):not([data-nosticky])").outerHeight() + $(".jumptoStore:not([data-nosticky])").outerHeight());
                $(id)[0].scrollIntoView(true);
                $(window).scrollTop($(window).scrollTop() - offset);
            }, (window.ActiveXObject || 'ActiveXObject' in window) ? 250 : 50);
        }
    }

    window.sdOffsetScroll = sdOffsetScroll;

    function sdOffsetMousewheel(ev)
    {
        $('body').data('scrolled', true);
    }

    $(document).on('click', '[href^="#"]', function (ev) {
        if ($(this).attr("href").match(/#.+/))
        {
            sdOffsetScroll($(this).attr("href"));
        }
    }).on('mousewheel', sdOffsetMousewheel);

    $(window).load(function (ev) {
        $(document).off('mousewheel', sdOffsetMousewheel);
        if (location.hash.match(/#.+/) && !$('body').data("scrolled"))
        {
            setTimeout(function () {
                sdOffsetScroll(location.hash);
            }, 200);
        }
        var match;
        if ((match = location.search.match(/p=(\d+)/)) && !location.hash && !$('body').data("scrolled"))
        {
            setTimeout(function () {
                sdOffsetScroll("#post" + match[1]);
            }, 200);
        }
    });
})();
;var liveStoreCategorySearch = {
    ...
    performAjaxRequest: function()
    {
        ...
        this.currentAjaxRequest = $.ajax({
            ...
            success: function(result)
            {
                ...
                if (result.store_count > 0 || result.category_count > 0)
                {
...
{
    if (typeof window[taq[i]] == 'function')
    {
        window[taq[i]]();
    }
}
