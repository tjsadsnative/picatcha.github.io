...
/************************** CONFIG SECTION **************************/
	...
	s.linkInternalFilters="javascript:,slickdeals"
...
2019:'3/10,11/3'}
	...
	/*Capturing Page URL*/
	s.eVar3=s.prop3=document.location.toString();
 
    /*Capture 404 error pages */
	if (s.pageType) 
	s.pageName="404:"+s.prop3;
...
utmmedium=s.Util.getQueryParam('utm_medium');
utmsource=s.Util.getQueryParam('utm_source');
utmterm=s.Util.getQueryParam('utm_term');
utmcontent=s.Util.getQueryParam('utm_content');
utmcampaign=s.Util.getQueryParam('utm_campaign');
gclid=s.Util.getQueryParam('gclid');
referrerid=s.Util.getQueryParam('referrerid');
v=s.Util.getQueryParam('v');
sdxt01=s.Util.getQueryParam('sdxt01');
sdxt02=s.Util.getQueryParam('sdxt02');
sdxt03=s.Util.getQueryParam('sdxt03');
sdxt04=s.Util.getQueryParam('sdxt04');
sdxt05=s.Util.getQueryParam('sdxt05');
sdxt06=s.Util.getQueryParam('sdxt06');
s.eVar60=s.Util.getQueryParam('utm_source');    
s.eVar61=s.Util.getQueryParam('utm_source');
if(!s.campaign)
{ if(utmmedium==""&&utmsource==""&&utmterm==""&&utmcontent==""&&utmcampaign==""&&gclid==""&&referrerid==""&&v==""&&sdxt01==""&&sdxt02==""&&sdxt03==""&&sdxt04==""&&sdxt05==""&&sdxt06=="")
{
s.campaign="";
}
else
{
s_camp=utmmedium.toLowerCase()+':'+utmsource.toLowerCase()+':'+utmcampaign.toLowerCase()+':'+utmcontent.toLowerCase()+':'+utmterm.toLowerCase()+':'+gclid.toLowerCase()+':'+referrerid.toLowerCase()+':'+v.toLowerCase()+':'+sdxt01.toLowerCase()+':'+sdxt02.toLowerCase()+':'+sdxt03.toLowerCase()+':'+sdxt04.toLowerCase()+':'+sdxt05.toLowerCase()+':'+sdxt06.toLowerCase();
if(s_camp=='::::::::')
{
s.campaign="";
}
...
s.campaign=s_camp;
	...
	s.prop2=s.eVar2=s.getPreviousValue(s.pageName,'gpv',''); 

	/* Capture Internal Campaign */
		if (!s.eVar4) {
			var icid = s.Util.getQueryParam('icid');
			if (icid) {
				s.eVar4 = icid;
			} 
		}
	  
var timeVar = s.getTimeParting('n','-7');
// need to check the hemisphere and time zone offset

timeVar = timeVar.split('|');
var hourOfDay=timeVar[3];
var dayOfWeek=timeVar[4];
...
s.prop5=s.eVar5= hourOfDay;
s.prop6=s.eVar6= dayOfWeek;
s.prop7=s.eVar7= typeOfDay
s.prop18=s.eVar46= today

/*Visitor Status*/
	  s.eVar31 = s.getNewRepeat(365);
	
	}
	s.doPlugins=s_doPlugins
  
/* Customization for SD to handle tracking failed image loads, compatible with 1.5 of the app measurement library; */
  if (s.version == "1.5")
  {
s.tb = function (c)
{
    var b,
            d, f;
    s.Pa() && 2047 < c.length && ("undefined" != typeof XMLHttpRequest && (b = new XMLHttpRequest,
            "withCredentials" in b ? d = 1 : b = 0),
    b || "undefined" == typeof XDomainRequest || (b = new XDomainRequest,
            d = 2),
    b && s.AudienceManagement && s.AudienceManagement.isReady() && (s.S ? b.pa = !0 : b = 0));
    !b && s.lb && (c = c.substring(0, 2047));
    !b && s.d.createElement && s.AudienceManagement && s.AudienceManagement.isReady() && (b = s.d.createElement("SCRIPT")) && "async" in b && ((f = (f = s.d.getElementsByTagName("HEAD")) && f[0] ? f[0] : s.d.body) ? (b.type = "text/javascript",
    ...
    ;
    b.onload = b.ub = function (e)
    {
      	/* Next 5 lines are custom  along with the parameter above */
        if (window.adobeSuccessCallback)
        {
          try {
            window.adobeSuccessCallback(e);
          } catch (e) { }
        }
        b.ra();
        s.$a();
    ...
    ;
    b.onabort = b.onerror = b.bb = function (e)
    {
      	/* Next 3 lines are custom  along with the parameter above */
        if (window.adobeFailedCallback)
        {
          try {
            window.adobeFailedCallback(e);
          } catch (e) { }
        }
        b.ra();
        (s.trackOffline || s.ja) && s.v && s.g.unshift(s.Za);
        s.v = 0;
        s.fa > s.J && s.Da(s.g);
        s.Z();
        s.ma(500)
    ...
    if (1 == d || 2 == d)
    {
        ...
        f = c.substring(0, e);
        e = c.substring(e + 1);
        e = e.replace(/&callback=[s-zA-Z0-9_.\[\]]+/, "");
        1 == d ? (b.open("POST", f, !0),
    ...
    b.abort && (s.la = setTimeout(b.abort, 5E3));
    s.Za = c;
    s.Ya = window["s_i_" + s.replace(s.account, ",", "_")] = b;
    if (s.useForcedLinkTracking && s.F || s.q)
        s.forcedLinkTrackingTimeout ||
        (s.forcedLinkTrackingTimeout = 250),
                s.aa = setTimeout(s.Z, s.forcedLinkTrackingTimeout)
}
;
  }
 ...
 */
s.inList = new Function("v", "l", "d", "tlc", "mt", "" + "var s=this,ar=Array(),i=0,d=(d)?d:',',v=(tlc)?v.toLowerCase():v,l=(" + "tlc)?l.toLowerCase():l,mt=(mt)?mt:0;if(typeof(l)=='string'){if(s.sp" + "lit)ar=s.split(l,d); else if(l.split)ar=l.split(d); else return-1;}" + " else ar=l;while(i<ar.length){if(!mt){if(v==ar[i])return true;} els" + "e if(mt){if(v.indexOf(ar[i])!=-1)return true;}i++;}return false;");
...
/*
* s.join: 1.0 - Joins an array into a string required plugin with crossVisitParticipation
*/
...
/*
* Plugin Utility: Replace v1.0 required plugin with crossVisitParticipation
*/
